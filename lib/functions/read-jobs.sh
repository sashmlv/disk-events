...
function read_jobs {
   ...
   readonly awk_cut_job_throttling='{ sub(/^<[0-9]+><[^>]*><[^>]*><[^>]*></, "" )}; match($0, /^[0-9]+[^>]*/ ) { str=substr($0, RSTART, RLENGTH); print str }' # job timeout throttling
   ...
   while read line; do
      ...
      if [[ ! -z "$line" ]]; then
         ...
         if [[ "$line" =~ $job_rgx ]]; then
            ...
            throttling=$(echo "$line" | awk "$awk_cut_job_throttling")
